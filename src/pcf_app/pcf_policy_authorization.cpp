...
status_code pcf_policy_authorization::post_app_sessions_handler(
  ...
  // Extract Session Key
  oai::pcf::app::session_binding_key session_key =
      oai::pcf::app::extract_session_key(context.getAscReqData());

  try {
    // Perform session binding using the session_binding_key
    m_event_sub.sm_session_binding(&session_key, &decision);
