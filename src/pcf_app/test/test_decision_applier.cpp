...
// Tests for decision_applier: the optimistic-concurrency CAS-retry loop
// shared by every Policy Authorization handler (create/modify/delete/
...
// sm_update_decision/tracker/max_retries are constructor-bound (real
// constructor injection), matching how pcf_policy_authorization builds one
// m_applier per instance and calls .apply() per request. decision_applier
// itself only commits -- notify is a separate, caller-driven step
// (pcf_policy_authorization::push_decision_change), so these tests don't
// exercise notify at all.
...
TEST(DecisionApplier, FirstAttemptCommitsAndRecordsToTracker) {
  ...
  const auto result = applier.apply(
      decision_apply_request{
          association_id, initial_base, /*initial_version=*/0, "as-1"},
      make_derive(9), committed_delta, problem_details, committed_version);
...
TEST(DecisionApplier, DeterministicDeriveFailureShortCircuits) {
  ...
  const auto result = applier.apply(
      decision_apply_request{association_id, initial_base, 0, "as-1"}, derive,
      committed_delta, problem_details, committed_version);
...
TEST(DecisionApplier, ExhaustsAfterMaxRetriesReturnsForbidden) {
  ...
  const auto result = applier.apply(
      decision_apply_request{association_id, initial_base, 0, "as-1"},
      make_derive(9), committed_delta, problem_details, committed_version);
