...
//------------------------------------------------------------------------------
bool launch::check_redundant_process(const std::string& exec_name) {
  // Strip the exec name from the path
  std::string process_name = fs::path{exec_name}.filename().string();

  std::string cmd = "pgrep -c " + process_name;

  std::string output = command::command_output(cmd);
...
//------------------------------------------------------------------------------
uint64_t command::index = 0;

//------------------------------------------------------------------------------
std::string command::command_output(const std::string& command) {
