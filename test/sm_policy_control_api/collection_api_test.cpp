...
class SMPoliciesCollectionApiTest : public ::testing::TestWithParam<::std::tuple<sm_policy::status_code, http_status_code_e>>
{
 public:
...
INSTANTIATE_TEST_SUITE_P(TestingDifferentState, SMPoliciesCollectionApiTest,
    Values(
        std::make_tuple(sm_policy::status_code::CREATED, HTTP_STATUS_CODE_201_CREATED),
        std::make_tuple(sm_policy::status_code::INVALID_PARAMETERS, HTTP_STATUS_CODE_400_BAD_REQUEST),
        std::make_tuple(sm_policy::status_code::CONTEXT_DENIED, HTTP_STATUS_CODE_403_FORBIDDEN),
        std::make_tuple(sm_policy::status_code::NOT_FOUND, HTTP_STATUS_CODE_500_INTERNAL_SERVER_ERROR),
        std::make_tuple(sm_policy::status_code::USER_UNKOWN, HTTP_STATUS_CODE_400_BAD_REQUEST)));

TEST_P(SMPoliciesCollectionApiTest, CreateNewSMPolicyAssociation) {
  std::string response_body;
  std::string response_headers;
  std::string url = fmt::format("{}sm-policies", sm_api_test.base_url);
  ...
  EXPECT_CALL(*sm_api_test.mock_pcf_smpc_inst, create_sm_policy_handler(_, _, _, _))
